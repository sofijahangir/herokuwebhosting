...
(function (global, factory) {
  ...
  const parseuri = function parseuri(str) {
    const src = str;
    const b = str.indexOf('[');
    const e = str.indexOf(']');
  ...
  function XMLHttpRequest$1(opts) {
    const { xdomain } = opts; // XMLHttpRequest can be disabled on IE
    ...
    _createClass(Polling, [
      ...
      },
      {
        ...
        value: function uri() {
          ...
          if (
            this.opts.port &&
            ((schema === 'https' && Number(this.opts.port) !== 443) ||
              (schema === 'http' && Number(this.opts.port) !== 80))
          ) {
            port = `:${this.opts.port}`;
          }
  ...
  const XHR = /* #__PURE__ */ (function (_Polling) {
    ...
    function XHR(opts) {
      ...
      if (typeof location !== 'undefined') {
        ...
        let { port } = location; // some user agents have empty `location.port`
    ...
    _createClass(WS, [
      ...
      },
      {
        ...
        value: function doOpen() {
          ...
          const uri = this.uri();
          const { protocols } = this.opts; // React Native only supports the 'headers' option, and will print a warning if anything else is passed
      ...
      },
      {
        ...
        value: function uri() {
          ...
          if (
            this.opts.port &&
            ((schema === 'wss' && Number(this.opts.port) !== 443) ||
              (schema === 'ws' && Number(this.opts.port) !== 80))
          ) {
            port = `:${this.opts.port}`;
  ...
  const Socket$1 = /* #__PURE__ */ (function (_Emitter) {
            ...
            transport.once('packet', function (msg) {
              ...
              if (msg.type === 'pong' && msg.data === 'probe') {
                ...
                _this4.transport.pause(function () {
                  if (failed) return;
                  if (_this4.readyState === 'closed') return;
                  cleanup();
    ...
    _createClass(Manager, [
      ...
      },
      {
        ...
        value: function open(fn) {
          ...
          const errorSub = on(socket, 'error', function (err) {
            self.cleanup();
